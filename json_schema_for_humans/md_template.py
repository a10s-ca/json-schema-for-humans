...
def restrictions_table(schema: SchemaNode) -> List[List[str]]:
    ...
    if schema.kw_min_length:
        restrictions.append(["**Longueur minimale**", str(schema.kw_min_length.literal)])
    if schema.kw_max_length:
        restrictions.append(["**Longueur maximale**", str(schema.kw_max_length.literal)])
    ...
    if schema.kw_multiple_of and schema.kw_multiple_of.literal_to_str:
        restrictions.append(["**Répétition de**", schema.kw_multiple_of.literal_to_str])
...
def array_items_restrictions(schema: SchemaNode) -> List[List[str]]:
    ...
    items_restrictions = [["Chaque item du tableau doit être", "Description"]]
    for i, item in enumerate(items_restriction_rules):
        item_label = item.name_for_breadcrumbs or f"Item de tableau {i}"
        item_html_id = item.html_id
...
class MarkdownTemplate(object):
    def __init__(self, config):
        ...
        if self.config.md_badge_as_image:
            ...
            self.badge_not_allowed = "Pas permis"
            self.badge_allowed = "Tout type permis"
            self.deprecated_badge = "Retiré"
            self.required_badge = ""
            self.optional_badge = ""
    ...
    def type_info_table(self, schema: SchemaNode) -> List[List]:
        ...
        type_info.append(["", ""])
        type_info.append(
            ["**Type**", "`combinaison`" if jinja_filters.is_combining(merged_schema) else f"`{schema_type}`"]
        )
        if not self.config.md_badge_as_image:
            type_info.append(["**Requis**", "Oui" if schema.is_required_property else "Non"])
        if jinja_filters.deprecated(self.config, merged_schema):
            type_info.append(["**Retiré**"])
            if self.config.md_badge_as_image:
        ...
        if schema_type == const.TYPE_OBJECT:
            type_info.append(["**Propriétés additionnelles**", self.additional_properties(merged_schema)])
        if default_value:
            type_info.append(["**Par défault**", f"`{default_value}`"])
            ...
            if sub_property.is_additional_properties:
                if sub_property.is_additional_properties_schema:
                    ...
                    if self.config.md_badge_as_image:
                        ...
                        additional_properties = f"[Chaque propriété additionnel doit respecter le schéma](#{html_id})"
                    break
                else:
    ...
    def array_restrictions(self, schema: SchemaNode) -> List[List[str]]:
        ...
        return [
            ...
            ["**Nombre d'items minimum**", str(schema.kw_min_items.literal) if schema.kw_min_items else "N/A"],
            ["**Nombre d'items maximum**", str(schema.kw_max_items.literal) if schema.kw_max_items else "N/A"],
            [
                "**Unicité**",
                "Vrai" if schema.kw_unique_items and schema.kw_unique_items.literal is True else "Faux",
            ],
            [
                "**Items additionnels**",
                "Vrai" if schema.array_additional_items else "Faux",
            ],
            [
                "**Validation du tuple**",
                (
